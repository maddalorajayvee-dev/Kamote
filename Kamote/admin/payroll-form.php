...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $overtimePay = $hourlyRate * $data['overtime_hours'] * $data['overtime_rate'];
...
if ($payroll) {
    $formData = $payroll;
    // Calculate overtime rate from stored data
    if ($payroll['overtime_pay'] > 0 && $payroll['overtime_hours'] > 0 && $payroll['base_salary'] > 0) {
        $hourlyRate = $payroll['base_salary'] / (8 * 22);
        $formData['overtime_rate'] = ($payroll['overtime_pay'] / ($hourlyRate * $payroll['overtime_hours']));
    } else {
        $formData['overtime_rate'] = 1.25;
    }
} else {
    $formData = [
        'employee_id' => '',
...
$overtimeRate = floatval($formData['overtime_rate'] ?? 1.25);
...
<head>
    ...
    <script>
        function calculateTotals() {
            ...
            const overtimeRate = parseFloat(document.querySelector('[name="overtime_rate"]').value) || 1.25;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const inputs = ['base_salary', 'overtime_hours', 'overtime_rate', 'allowances', 'bonuses', 'deductions'];
            inputs.forEach(name => {
    ...
    <main class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div class="px-4 py-6 sm:px-0">
            <div class="mb-6 flex items-center justify-between">
                <a href="<?php echo base_url('admin/payroll.php'); ?>" class="text-primary-600 hover:text-primary-800 text-sm font-medium mb-4 inline-flex items-center gap-2">
                    ← Back to Payroll
                </a>
                ...
                <form method="POST" class="space-y-6">
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="number" name="overtime_rate" value="<?php echo htmlspecialchars($formData['overtime_rate'] ?? '1.25'); ?>" min="1" step="0.25"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent" />
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="bonuses" value="<?php echo htmlspecialchars($formData['bonuses'] ?? '0'); ?>" min="0" step="0.01"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent" />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Deductions (Tax, SSS, PhilHealth, etc.)</label>
                            <input type="number" name="deductions" value="<?php echo htmlspecialchars($formData['deductions'] ?? '0'); ?>" min="0" step="0.01"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent" />
                        </div>
                    </div>
